...
igg::Image::Image(const std::shared_ptr<IoStrategy>& strategy_ptr):strategy_ptr{strategy_ptr}{}

igg::Image::Image(int rows, int cols, const std::shared_ptr<IoStrategy>& strategy_ptr):strategy_ptr{strategy_ptr}{
    rows_ = {rows};
    cols_ = {cols};
}
...
bool igg::Image::ReadFromDisk(const string& file_name){
    ...
    try{
        igg::ImageData image = strategy_ptr->Read(file_name);
...
void igg::Image::WriteToDisk(const string& file_name){
    ...
    strategy_ptr->Write(file_name, data);
}
